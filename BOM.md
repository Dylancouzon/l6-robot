...
| 3 | Camera | Arducam IMX291 USB camera module (B0200) | 60 | Bare 38×38 mm UVC board, 1080p30, 100° lens, good low light. Screws behind the shell's lens opening; works with `cv2.VideoCapture` on Jetson, no drivers. Sold by Arducam on Amazon. |
...
| | | **Required total** | **~$372** | Excludes optional battery and level shifter. |
...
- Use a UVC USB camera unless you already know you want the CSI camera driver work. A desk webcam like the Logitech C920 is fine for laptop development, but its housing does not fit inside the shell.
...
3. The app captures at 1280×720. Over USB 2.0 the camera only reaches 30 fps in MJPG, so set `cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))` if frames come in slow.
4. Install PyTorch and Ultralytics from NVIDIA's JetPack-compatible packages or container images, not stock desktop CUDA wheels.
5. Keep FastEmbed and `onnx-asr` on CPU ONNX Runtime unless a tested Jetson GPU path is added.
6. Keep `pyobjc-core` macOS-only in `pyproject.toml`.
7. Keep `sounddevice` for laptop development. The demo interface should use phone/tablet audio upload.
8. Serve the browser UI over HTTPS when using a phone mic. Mobile browsers require a secure context for `getUserMedia`.
9. Enable Jetson SPI before driving APA102/DotStar LEDs.
10. Treat TensorRT export as an optimization step after the base demo is working.
