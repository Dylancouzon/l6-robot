...
- **DROP THIS VIEW deletes just the picture you are looking at — taught views and sightings alike.** This is the cure for a single bad teach (a blurred crop, or one that caught the desk instead of the mug) and equally for a bad auto-saved photo (a stale box, a hand over the object), either of which otherwise sits in memory forever. Tap the picture until the bad one is showing, then drop it. Dropping a sighting removes only that photo; the button hides on an object's *last taught view*, because dropping that would forget the whole object — FORGET is the honest button for that.
